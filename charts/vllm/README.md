...
ifconfig --> get the interface name, and set GLOO_SOCKET_IFNAME and NCCL_SOCKET_IFNAME to it
...
docker run -it --rm \
    ...
    kalavai/ray-vllm-rocm:v0.12.0

# on all nodes RAY start

export HF_TOKEN=XXXX
ray start --node-ip-address=66.49.195.11 --head
ray start --address=66.49.195.11:6379 --node-ip-address=66.49.195.13
ray start --address=66.49.195.11:6379 --node-ip-address=66.49.195.14

# on head
vllm serve Qwen/Qwen2.5-3B \
  --host 0.0.0.0 --port 8080 --distributed-executor-backend="ray" --pipeline-parallel-size=3
...
curl "http://66.49.195.11:8080/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -d '{
        "model": "Qwen/Qwen2.5-3B",
        "messages": [
...
```bash
bash build_images.sh --latest --version 0.11.2
```

This script will:
- Build/push a vLLM-rocm base image
- Build/push a ray-vllm-cuda image with the corresponding version as tag ("v0.11.2")
- Build/push a ray-vllm-rocm image with the corresponding version as tag ("v0.11.2")
